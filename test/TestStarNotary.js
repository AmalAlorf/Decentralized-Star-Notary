...
it('can add the star name and star symbol properly', async() => {
    ...
    let lookUpStar = await instance.lookUptokenIdToStarInfo(starId);
    assert.equal(ContractName, 'Star Token');
    assert.equal(ContractSymbol, 'USD');
    assert.equal(lookUpStar, 'My Star');
...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('StarTest1', starId1, { from: user1 });
    await instance.createStar('StarTest2', starId2, { from: user2 });
    // 2. Call the exchangeStars functions implemented in the Smart Contract
    await instance.exchangeStars(starId1, starId2);
    //Get the stars owners
...
it('lets a user transfer a star', async() => {
    ...
    let user2 = accounts[1];
    let tokenId = 9;
    await instance.createStar('The Star', tokenId, { from: user1 });
